...
module.exports.get_all_orders = async(req, res) => {
    ...
    try {
        const allOrders = await Order.find({$or: [{orderType: "Instant-Order"}, {orderType: "Shipment"}]}).sort("-receiptNo").skip(10 * nextPage).limit(10)
...
module.exports.create_order = async(req, res) => {
    ...
    const {customer, items, orderType, payType, createdBy, shipmentFee, reservationDate, discount, isReserved} = req.body
...
module.exports.edit_order = async(req, res) => {
    ...
    if (!_id || !items || !customer || !total || !paid)
      return res.status(400).json({ error: "Please include necessary info" });
    try {
...
module.exports.revoke_debt = async(req, res) => {
    if(!req.body) return res.json({msg: "Please include required info as JSON"})
    const {_id} = req.body
    if(!_id) return res.status(400).json({error: "Please include necessary info"})
    try {
        const modOrder = await Order.findOneAndUpdate
        (
            {_id},
            {
                paidTotal: 0,
